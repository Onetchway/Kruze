...
/**
 * A guard's identity is created by their vendor, never self-signed-up —
 * this screen only lets an already-onboarded guard set up their own
 * mobile login, by re-proving who they are (global guard ID + the phone
 * number the vendor registered) and picking a password.
 */
export default function ClaimAccountScreen({ onBackToLogin }: { onBackToLogin: () => void }) {
  const [globalGuardId, setGlobalGuardId] = useState("");
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      const result = await api.claimAccount({ globalGuardId, phone, email, password });
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.subtitle}>Ask your fleet office for your Guard ID (e.g. KZ-GRD-000001) if you don&rsquo;t have it.</Text>

      <TextInput style={styles.input} placeholder="Guard ID (e.g. KZ-GRD-000001)" value={globalGuardId} onChangeText={setGlobalGuardId} autoCapitalize="characters" />
