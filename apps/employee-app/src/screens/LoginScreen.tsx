...
export default function LoginScreen({ onGoToSignup }: { onGoToSignup: () => void }) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (!result.accessToken || !result.organisationId || !result.role) {
        setError("This account belongs to multiple organisations — not supported in this app yet.");
        return;
      }
      if (result.role !== "EMPLOYEE") {
        setError("This app is for employees. Your account is not an employee account.");
        return;
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Employee transport</Text>
      ...
      <TouchableOpacity onPress={onGoToSignup} style={{ marginTop: 20 }}>
        <Text style={styles.link}>New here? Request transport access</Text>
      </TouchableOpacity>
    </View>
  );
}
