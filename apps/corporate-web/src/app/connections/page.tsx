...
function ConnectionsContent() {
  ...
  const isCorporate = !!session && CORPORATE_ROLES.includes(session.role);
  ...
  return (
    <>
      <h2 style={{ marginTop: 0 }}>{isCorporate ? "Vendors" : "Corporates"}</h2>
      <p style={{ color: "var(--text-muted)" }}>
        {isCorporate
          ? "Connect a vendor or fleet operator by their Kruze ID. They must accept before their fleet becomes eligible for auto-assignment on your trips."
          : "Connect a corporate by their Kruze ID, or accept an invite a corporate has already sent you. Once active, your compliant fleet becomes eligible for their auto-plan."}
      </p>

      <div className="card">
        <form onSubmit={handleInvite} style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div className="field" style={{ marginBottom: 0, minWidth: 220 }}>
            <label>Kruze ID (e.g. KZ-VND-000001)</label>
            <input value={globalOrgId} onChange={(e) => setGlobalOrgId(e.target.value)} required placeholder="KZ-VND-000001" />
          </div>
